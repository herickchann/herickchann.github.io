...
_flutter.loader.load({
  onEntrypointLoaded: async function (engineInitializer) {
    ...
    // Flutter's generated service worker is deprecated. Keep our small,
    // base-path-aware worker so GitHub Pages remains installable and can reopen
    // after the first successful load.
    if ('serviceWorker' in navigator) {
      const registerWorker = function () {
        navigator.serviceWorker
          .register(new URL('pwa_service_worker.js', document.baseURI))
          .catch(function (error) {
            console.warn('PokeBindr service worker registration failed:', error);
          });
      };
